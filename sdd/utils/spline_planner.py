    ...
    for ti in range(len(disc_xy_)):
        ...
        if feasible_horizon <= horizon_:
            ...
            if method == 'conformal controller':
                reward, loss = eval_reward_numba_conformal_controller(goal, traj, vel, pred_splines, human_rad, sd_obs_interp, lamda, False)
            if method == 'proactive conformal controller':
                reward, loss = eval_reward_numba_conformal_controller(goal, traj, vel, pred_splines, human_rad, sd_obs_interp, lamda, True)
            if method == 'aci':
