...
#		if y_low[j] <= -1 and y_up[j] >= 0:
#			if x_low[j] <= -1 and x_up[j] >= 0:
#				patch[j] = patch[j-1] # TODO: Fix kluge
#			else:
#				patch[j] = torch.cat([template[y_low[j]:-1, x_low[j]:x_up[j]], template[0:y_up[j]+1, x_low[j]:x_up[j]]], axis=0)
#		elif x_low[j] <= -1 and x_up[j] >= 0:
#			patch[j] = torch.cat([template[y_low[j]:y_up[j], x_low[j]:-1], template[y_low[j]:y_up[j], 0:x_up[j]+1]], axis=1)
#		patch[j] = torch.nn.functional.interpolate(patch[j][None, None, :,:], size=(H, W), mode='bicubic', align_corners=False).squeeze()
	return patch
