...
PATH_TO_ANA = "/home/aa/Code/conformal-decision/sdd"
DEFAULT_PATH = "/home/aa/Code/conformal-decision/sdd/ynet_additional_files/data/SDD/"
VIDEO_PATH = '/checkpoints/ec/SDD/videos/'

def load_SDD(scene, path=DEFAULT_PATH, video_path=VIDEO_PATH, mode='train', load_frames=False):
    '''
    ...
    if load_frames:
        ...
        frames = [im for im in tqdm(vid.iter_data())]
    else:
        frames = None
...
def load_SDD_old(scene='nexus_4', path=DEFAULT_PATH, video_path=VIDEO_PATH, mode='train'):
    '''
    Loads data from Stanford Drone Dataset. Makes the following preprocessing:
    -filter out unnecessary columns (e.g. generated, label, occluded)
    -filter out non-pedestrian
    ...
    path = os.path.join(PATH_TO_ANA, path)
    path = os.path.join(path, mode)
    SDD_cols = ['trackId', 'xmin', 'ymin', 'xmax', 'ymax', 'frame', 'lost', 'occluded', 'generated', 'label']
    data = []
    print('loading ' + mode + ' data')
...
def get_reference_image(scene, path=DEFAULT_PATH, mode='train'):
    # Get reference image
